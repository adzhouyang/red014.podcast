// ─── Hermes Script Provider ────────────────────────
// PRD 6.6: Optional script model via OpenAI-compatible endpoint
//
// Env: HERMES_API_KEY, HERMES_BASE_URL (both required), HERMES_MODEL (optional)
...
export const hermesProvider: ScriptProvider = {
  name: "hermes",
  models: [MODEL],
  ...
  async generateScript(input: ScriptGenerationInput): Promise<PodcastScript> {
    if (!API_KEY) {
      throw new Error(
        "HERMES_API_KEY not set. Set it in .env.local or environment.",
      );
    }
    if (!BASE_URL) {
      throw new Error(
        "HERMES_BASE_URL not set. Set it in .env.local or environment.",
      );
    }
  ...
  async regenerateSegment(input: SegmentRegenerationInput): Promise<Segment> {
    if (!API_KEY) throw new Error("HERMES_API_KEY not set.");
    if (!BASE_URL) throw new Error("HERMES_BASE_URL not set.");
